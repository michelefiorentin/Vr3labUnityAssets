...
public class ArrowRotationListener : MonoBehaviour
{
    ...
    public Vector3 myaxis = new Vector3(0, 1, 0);

    [Tooltip("(Sperimetal)Set interpolation to target")]
    private bool LerpOn = false; // lerping function is sperimental and not works

    [Tooltip("Duration of teh interpolation to target")]
    private float lerpDuration = 4f; // considering nemea  = 1 sec
    ...
    {
        original = transform.localEulerAngles.y;
        target = wind.Angle;
