...
public class DayNightController : MonoBehaviour {
    ...
    public System.Collections.Generic.List<Light> nightlightslist;
    private List<float> nightlightInitialIntensitylist =new List<float>();
    ...
    [Range(0, 1)]
    public float startnightlights = 0.1f;
    [Range(0, 1)]
    public float endnightlights = 0.8f;
    ...
    void Start() {
        sunInitialIntensity = sun.intensity;
                //nightlightInitialIntensity = nightlight.intensity;
        for (int i = 0; i < nightlightslist.Count; i++)
        {
            nightlightInitialIntensitylist.Add(nightlightslist[i].intensity);
        }
    ...
    void UpdateNightLights()
    {
                ...
                float intensityMultiplier = 1;
        // Set intensity to 1 during the night night.
        if (currentTimeOfDay <= startnightlights || currentTimeOfDay >= endnightlights)
        {
            intensityMultiplier = 1;
        }
        else intensityMultiplier = 0;
        /*
        // Fade in the sun when it rises.
        else if (currentTimeOfDay <= 0.25f)
        {
        ...
        */
        ...
        for (int i = 0; i < nightlightslist.Count; i++)
        {
            nightlightslist[i].intensity = nightlightInitialIntensitylist[i] * intensityMultiplier;
        }
